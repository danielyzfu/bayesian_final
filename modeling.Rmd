...
```{r}
summary(baseline_logit)$coef |> xtable()
```
```{r}
beatspy_clean = m3_df
```
...
```{r}
summary(firm_re)$coef |> xtable()
```

```{r}
VarCorr(firm_re) |> as.data.frame() |> xtable()
```
...
cv_results = map_dfr(folds, function(f){
  ...
  pred_baseline = predict(baseline_mod, newdata = test, type = "response")
  ...
  pred_re = predict(re_mod, newdata = test, type = "response")
...
```{r}
auc_table <- tibble(
  model = c("Firm Mixed Effects", "Sector Fixed Effect", "Baseline Logistic", "Fei"),
  auc = c(
    auc(cv_results$true, cv_results$mixed),
    auc(cv_results$true, cv_results$pooled),
    auc(cv_results$true, cv_results$baseline),
    auc(cv_results$true, cv_results$fei)
  )
)
...
```{r}
vif(sector_fe) |> as.data.frame() |> xtable()
```

### Influence/Outliers

```{r}
png("images/cooks.png",height=2000,width=2000,res=300)
...
```{r}
cooks_d = cooks.distance(sector_fe)
cooks_threshold = 4 / (nrow(m3_df) - length(coef(sector_fe)))

influential = which(cooks_d > cooks_threshold)
test_cook = m3_df[-influential, ]
cook_sector_fe = glm(
  beat_spy ~ log_pe + div_yield + gics_sector_name,
  data = test_cook,
  family = binomial(link = "logit")
)
summary(cook_sector_fe)$coef
```
```{r}
cv_df = test_cook |>
  fold(k = 10, cat_col = "gics_sector_name")
...
cook_cv_results = map_dfr(folds, function(f){
...
cook_cv_results = cook_cv_results %>%
  mutate(
    true_bin = ifelse(true == "yes", 1, 0)
  )
```
...
```{r}
cook_auc_table <- tibble(
  model = c("Firm Mixed Effects", "Sector Fixed Effect", "Baseline Logistic"),
  auc = c(
    auc(cook_cv_results$true, cook_cv_results$mixed),
    auc(cook_cv_results$true, cook_cv_results$pooled),
    auc(cook_cv_results$true, cook_cv_results$baseline)
  )
)

auc_table |> arrange(desc(auc)) |> xtable()
...
```{r}
png("images/cr.png",height=2000,width=2000,res=300)
crPlots(sector_fe)
dev.off()
```
...
```{r}
summary(bayes_baseline)$fixed |> xtable()
```
...
ggsave("images/baseline_trace.png")
```
...
ggsave("images/baseline_acf.png")
```
...
mcmc_trace(draws_bayes_fe,
           pars = c("b_log_pe", "b_div_yield", "b_Intercept","b_gics_sector_nameInformationTechnology"))
ggsave("images/sector_trace.png")
```
...
```{r}
loo_compare(loo(bayes_baseline),
            loo(bayes_fe)) |>
  as.data.frame() |>
  xtable()
```
...
```{r}
summary(bayes_weak)$fixed |> xtable()
```

```{r}
summary(bayes_regularized)$fixed |> xtable()
```

```{r}
loo_fe  <- loo(bayes_fe)
loo_weak <- loo(bayes_weak)
loo_reg <- loo(bayes_regularized)
loo_compare(loo_fe, loo_weak, loo_reg)
```

```{r}
pp_check(bayes_weak)
ggsave("images/weak_pp.png")
```

```{r}
pp_check(bayes_regularized)
ggsave("images/reg_pp.png")
```

```{r}
kfold_fe <- kfold(bayes_fe, K = 10)
print(kfold_fe)
```

```{r}
new_stock <- data.frame(
  div_yield = seq(0, 5),
  log_pe = 3,
  gics_sector_name = "Consumer Discretionary"
)
pred_prob <- posterior_epred(bayes_fe, newdata = new_stock)
pred_df = new_stock |>
  mutate(mean = apply(pred_prob, 2, mean),
         lower = apply(pred_prob, 2, quantile, 0.025),
         upper = apply(pred_prob, 2, quantile, 0.975))

ggplot(pred_df, aes(x = div_yield, y = mean)) +
  geom_line()+
  geom_ribbon(aes(ymin = lower, ymax = upper),
              alpha = 0.5) +
  labs(
    x = "Dividend Yield",
    y = "Predicted Probability of Beating SPY",
    title = "Consumer Discretionary firm with log(P/E) = 3"
  )
ggsave("images/prediction.png")
```
