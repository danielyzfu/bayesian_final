...
sector_fe_interact = glmer(
  beat_spy ~ (log_pe + div_yield) | gics_sector_name,
  data = m3_df,
  family = binomial(link = "logit")
)

summary(sector_fe_interact)
```
...
### Influence/Outliers
```{r}
plot(cooks.distance(pooled_fe), type = "h")
abline(h = 4/length(fitted(pooled_fe)), col = "red")
```

### Leverage
```{r}
plot(hatvalues(pooled_fe), type = "h")
abline(h = 3*(length(coef(pooled_fe))+1)/length(fitted(pooled_fe)), col = "red")
```

### Residuals

```{r}
plot(residuals(pooled_fe, type = "deviance"),
     col = rgb(0, 0, 0, 0.25))
```
...
ggplot(
  data.frame(
    fitted = fitted(pooled_fe),
    resid = residuals(pooled_fe, type = "deviance")
  ),
